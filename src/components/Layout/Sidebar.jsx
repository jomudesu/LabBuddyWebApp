  ...
  return (
    <aside className="w-64 bg-slate-50/5 shadow-[4px_0_24px_rgba(0,0,0,0.02)] flex flex-col border-r border-gray-100 z-50 relative">
      ...
      <nav className="flex-1 px-4 mt-2">
        {menuItems.map((item, index) => (
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon size={20} className={`transition-transform duration-300 ${isActive ? 'scale-110 text-blue-600' : 'group-hover:scale-110 group-hover:-rotate-3'}`} />
      ...
      <div className="p-5 border-t border-gray-100 bg-white">
        ...
        <button onClick={handleLogout} className="group w-full flex items-center justify-center px-4 py-2.5 text-sm font-bold text-red-600 bg-transparent hover:bg-red-50 hover:text-red-700 rounded-xl transition-all duration-300 border border-transparent hover:border-red-100 hover:shadow-sm">
