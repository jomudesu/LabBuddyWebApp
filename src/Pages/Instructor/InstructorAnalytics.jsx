...
import { BarChart3, Search, Filter, LayoutGrid, CheckCircle2, Minus, TrendingUp, Award, AlertCircle, FlaskConical, Mail } from 'lucide-react';
  ...
  const gradebook = useMemo(() => {
    ...
    return filteredStudents.map(student => {
      ...
      experiments.forEach(exp => {
        ...
        if (record && record.status === 'completed') { completions[exp.id] = true; completedCount++; } 
        else { completions[exp.id] = false; }
      });
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {gradebook.length > 0 ? gradebook.map((student) => (
                <tr key={student.id} className="hover:bg-purple-50/20 group">
                  ...
                  {experiments.map(exp => (
                    <td key={exp.id} className="px-6 py-3.5 border-r border-slate-100 text-center"><div className="flex justify-center items-center h-full">{student.completions[exp.id] ? <div className="w-7 h-7 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center border border-emerald-200" title="Completed"><CheckCircle2 size={16} strokeWidth={2.5} /></div> : <div className="w-7 h-7 bg-slate-50 text-slate-300 rounded-full flex items-center justify-center border border-slate-200" title="Not Started"><Minus size={16} strokeWidth={2.5} /></div>}</div></td>
                  ))}
                </tr>
